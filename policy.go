...
type apiPolicy struct {
	...
	InternetServiceName    []namedItem `json:"internet-service-name"`
	InternetServiceSrc     string      `json:"internet-service-src"`
	InternetServiceSrcName []namedItem `json:"internet-service-src-name"`
}
...
func (c *Client) ListPolicies(ctx context.Context, vdom string, opts ...ListOption) ([]Policy, error) {
	...
	for i, p := range items {
		policies[i] = Policy{
			...
			InternetServiceName:    namesOf(p.InternetServiceName),
			InternetServiceSrc:     p.InternetServiceSrc,
			InternetServiceSrcName: namesOf(p.InternetServiceSrcName),
		}
	}
